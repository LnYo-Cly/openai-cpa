    ...
    methods: {
        ...
        async fetchConfig() {
            ...
            try {
                ...
                const legacyGrok = this.config.grok2api_mode;
                if (legacyGrok && (!this.config.grok2api.admin_password || this.config.grok2api.api_url === 'http://host.docker.internal:8000')) {
                    this.config.grok2api.enable = legacyGrok.enable ?? this.config.grok2api.enable;
                    this.config.grok2api.api_url = legacyGrok.api_url || this.config.grok2api.api_url;
                    this.config.grok2api.admin_password = legacyGrok.password || this.config.grok2api.admin_password;
                    if (legacyGrok.target === 'grok_web') this.config.grok2api.import_sso_as_grok_web = true;
                }
                delete this.config.grok2api_mode;
