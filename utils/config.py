...
def reload_all_configs(new_config_dict=None):
    ...
    DISCARD_ON_DOWNGRADE = safe_bool(_c.get("discard_on_downgrade", True), default=True)
    ...
    raw_push_format = str(_sub2api.get("push_format", "agent_identity") or "agent_identity").strip().lower()
    ...
    SUB2API_AGENT_IDENTITY_RUNTIME_RECOVERY = safe_bool(
        _sub2api.get("agent_identity_runtime_recovery", True), default=True
    )
    SUB2API_AGENT_IDENTITY_RECOVERY_CROSS_ACCOUNT = safe_bool(
    ...
    PLUS_ACT_MAX_CONCURRENT = safe_int(_plus.get("max_concurrent", 1), 1)
    PLUS_ACT_RETRY_LIMIT = safe_int(_plus.get("retry_limit", 3), 3)
    PLUS_ACT_RETRY_DELAY = safe_int(_plus.get("retry_delay_sec", 30), 30)
    ...
    PLUS_ACT_SMS_POOL_FILE = str(_plus.get("sms_pool_file", "data/sms_pool.txt")).strip()
    PLUS_ACT_PUSH_TARGETS = _plus.get("push_targets", [])
