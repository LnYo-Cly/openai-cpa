...
class NewAPIClient:
    """NewAPI 渠道管理客户端 — 将注册账号追加到 Codex (type=57) 多Key渠道"""
    ...
    def _build_codex_key(self, token_data: dict) -> str:
        """从 token_data 构建 Codex 渠道所需的单个 JSON key"""
        key_obj = {
    ...
    def list_codex_channels(self) -> Tuple[bool, Any]:
        ...
        try:
            ...
            codex_channels = [ch for ch in channels if ch.get("type") == self.CODEX_CHANNEL_TYPE]
            # 返回精简信息
            result = []
            for ch in codex_channels:
                key_str = ch.get("key", "")
