...
class TurnstileAPIServer:
    ...
    async def _initialize_browser(self) -> None:
        ...
        for i in range(self.thread_count):
            ...
            if self.browser_type in ['chromium', 'chrome', 'msedge'] and playwright:
                # 原仓库逻辑；chromium 不传 channel（避免错误浏览器路径）
                launch_kwargs = {
    ...
    async def _find_and_click_checkbox(self, page, index: int):
        ...
        try:
            ...
            if iframe_locator:
                try:
                    # 不要用 element_handle() 默认 30s 超时；改用 frame_locator 快速点 checkbox
                    frame_selectors = [
                        'iframe[src*="challenges.cloudflare.com"]',
                        'iframe[src*="turnstile"]',
                        'iframe[title*="widget"]',
                    ...
                    for fsel in frame_selectors:
                        ...
                        for selector in (
                            ...
                            try:
                                target = fl.locator(selector).first
                                await target.click(timeout=1500, force=True)
                                if self.debug:
                                    logger.debug(
                                        f"Browser {index}: clicked inside iframe via frame_locator "
                                        f"{fsel} -> {selector}"
                                    )
                                clicked = True
                                return True
                    ...
                    # fallback: 直接点 iframe 本体（短超时）
                    try:
                        await iframe_locator.click(timeout=1500, force=True)
                        if self.debug:
                            logger.debug(f"Browser {index}: clicked turnstile iframe element")
                        return True
                    except Exception as e:
                        if self.debug:
                    ...
                    if self.debug:
                        logger.debug(f"Browser {index}: Failed to access iframe content: {str(e)[:160]}")
    ...
    async def _try_click_strategies(self, page, index: int):
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                # page.evaluate 常返回 None，不能当成功，否则会跳过后续策略
                if result is True:
                    if self.debug:
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None, proxy: Optional[str] = None):
        """Solve the Turnstile challenge (original-repo style).

        Extra optional proxy= is only for host-app integration.
        """
        ...
        try:
            try:
                ...
                try:
                    if hasattr(browser, "is_connected") and not browser.is_connected():
                        ...
                        await save_result(
                            task_id,
                            "turnstile",
                            {"value": "CAPTCHA_FAIL", "elapsed_time": 0, "error": "browser_disconnected"},
                        )
                        return
                except Exception as e:
                    if self.debug:
                ...
                if selected_proxy is None and self.proxy_support:
        ...
        """
                        ...
                        for attempt in range(poll_attempts):
                            try:
                                ...
                                if count == 0:
                                    if not reinjected and attempt >= 8 and round_i < rounds:
                                        try:
                                            await self._inject_captcha_directly(
                                                page, sitekey, action or "", cdata or "", index
                                            )
                                            reinjected = True
                                            if self.debug:
                                                logger.debug(f"Browser {index}: re-injected Turnstile widget")
                                        except Exception as reinj_err:
                                            if self.debug:
                                                logger.debug(f"Browser {index}: re-inject failed: {reinj_err}")
                                    ...
                                    try:
                                        ...
                                        if token:
                                            ...
                                            await save_result(
                                                task_id,
                                                "turnstile",
                                                {"value": token, "elapsed_time": elapsed_time, "round": round_i},
                                            )
                                            return
                                    except Exception as e:
                                        if self.debug:
                                ...
                                if attempt > 1 and attempt % 2 == 0 and click_count < max_clicks:
                                    ...
                                    if self.debug:
                                        logger.debug(
                                            f"Browser {index}: click #{click_count}/{max_clicks} success={click_success}"
                                        )
                        ...
                        logger.error(f"Browser {index}: Error solving Turnstile round {round_i}: {e}")
                    finally:
...
def parse_args():
    ...
    parser.add_argument('--browser_type', type=str, default='camoufox', help='Specify the browser type for the solver. Supported options: camoufox, chromium, chrome, msedge (default: camoufox)')
